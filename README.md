...
- [17 Запахи и эвристические правила](#17-запахи-и-эвристические-правила)
    ...
    - [17.6 Тесты](#176-тесты)
        ...
        - [T9: Тесты должны работать быстро](#t9-тесты-должны-работать-быстро)
...
В открытой реализации пользователь вынужден работать только с прямоугольными координатами. И такое решение раскрывает
...
функционал, который для текущего использования не следует использовать. Например, `java.util.Map` предоставляет методы
очистки контейнера(`Map#clear`), даже тогда когда это не нужно.

Можно объекты стороннего интерфейса хранить внутри класса, в котором он используется:
...
3. Не нужно передавать эти переменные в виде параметра метода, а преобразовать эти переменные в поля класса
...
Большая часть шаблонного кода посредников может автоматически обрабатываться вспомогательными средствами, например в
`Spring AOP`.
...
соответствующие принципу единоответсвенности, которые проще тестировать. А чем больше тестов напишем, тем больше
...
* При написании кода реализации многопоточности возникают специфические сложности, принципиально отличающиеся от
  сложностей однопоточного кода (и часто превосходящие их)
...
Читатель - читающий из связанного ресурса.

Писатель - периодические обновляет связанный ресурс.
...
* Блокировка на стороне сервера - на стороне сервера создается метод, который блокирует сервер, вызывает все методы,
...
Лишь несколько из тысячи возможных путей выполнения многопоточного кода, плохо написанной секции, приводят к
фактическому отказу. Вероятность выбора сбойного пути ничтожно мала, что усложняет выявление ошибок.
...
Программная инструментовка кода возможно с помощью `Aspect-Oriented Framework`, `CGLIB` или `ASM`.
...
Например, ручное управление `serialVersionUID` бывает необходимо, но рискованно. Иногда отключение предупреждения
...
полным! Например, константы, переменные и вспомогательные функции конкретной реализации, должны храниться в производных
классах и абстрактный класс о них ничего не должен знать.

Данное правило относится к исходным файлам, компонентам и модулям.

Разделение абстракций - одна из самых сложных задач, решаемая разработчиками. Если выбор сделан неверно, не надейтесь,
что вам удастся найти простое обходное решение.

### G7: Базовые классы, зависящие от производных

Базовые классы относящиеся к более высокому уровню абстракций, ничего не должны знать о своих производных классах.
...
Если используются два и более класса из паркета, импортируйте весь пакет командой `import package.*;`

Начало модуля не должно загромождаться 80-строчным списком директив импорта. Он должен быть точной и лаконичной
конструкцией, показывающая с какими пакетами работают.

Конкретные директивы импорта определяют жесткие зависимости(требуют использовать импортированный класс), обобщенные
директивы импорта - нет.

Обобщенные директивы импорта иногда становятся причиной конфликтов имен и неоднозначностей.

### J2: Не наследуйте от констант

Есть подход, когда константы размещают в интерфейсе и делают реализацию от этого интерфейса, что бы при использовании
константы, не указывать класс/интерфейс, где она располагается:
...
При этом наследование используется не по своему назначению. Если требуется обойти языковое правило видимости, можно
использовать статистическое импортирование `import static PayrollConstants.*;`

### J3: Константы против перечислений

Начиная `Java 5` появились `enum`, которым можно заменить старый трюк с `public static final int`. Подробнее
в `Java. Эффективное программирование`

## 17.5 Имена

### N1: Используйте содержательные имена

Позаботьтесь о том, чтобы имена были содержательными. Смысл может измениться в ходе развития проекта, почаще
переосмысливайте уместность выбранных имен.

Имена в программах на 90% определяют удобочитаемость кода. НЕ жалейте времени для выбора осмысленного имени.

Код с плохим выбором имен и "волшебными цифрами":
...
Переработанные имена и вынесение содержательные методы часть кода:
...
При прочтении подобного кода, позволяет понять примерную реализацию вспомогательных используемых методов.

### N2: Выбирайте имена на подходящем уровне абстракции

Не используйте имена, передающие информацию о реализации. Имена должны отражать уровень абстракции, на котором работает
класс или функция.
...
В данном примере, если для установки связи используется не только коммутируемое подключение? Концепция телефонных
номеров относится не к тому уровню абстракции. Правильнее будет так:
...
### N3: По возможность используйте стандартную номенклатуру

Имена проще понять, если они основаны на существующих конвенциях или стандартных обозначений. Например, при
использовании паттерна **Декоратор** можно добавить к названию класса слово `Decorator`.
...
В коде метода выше название слишком общее. И когда в функции `doRename` вызывается функция `renamePage` - это усугубляет
понимание функции. Лучше было её назвать `renamePageAndOptionallyAllReferences`, функция вызывается из одной точки
модуля, поэтому её документирующая ценность имени превышает длину.

### N5: Используйте длинные имена для длинных областей видимости

Если область видимости переменной составляет около пяти строк, то переменной можно присвоить имя например `i` или `j`.
Чем больше область видимости у переменной, тем более длинным и точным должно быть имя.

### N6: Избегайте кодирования

Ничего не должно кодироваться в именах: ни о типе, ни об области, ни об информации об проекте или подсистеме.
Современные среды разработки позволяют получить всю нужную информацию.

### N7: Имена должны описывать побочные эффекты

Не скрывайте побочные эффекты за именами, используя простые глаголы.
...
Данная функция может создать объект, если он не был создан, потом вернет его. Правильнее было бы назвать её
`createOrReturnOos`.
...
Тестовый пакет должен тестировать все, что может сломаться. Если в системе остались условия или вычисления, не
проверенные тестами, значит количество тестов нехватает.

### T2: Используйте средства анализа покрытия кода

Средства анализа покрытия сообщают о пробелах тестирования и упрощают поиск кода с недостаточно полным тестированием.
...
### T4: Отключенные тест как вопрос

Иногда нет уверенности в подробностях поведения системы, потому что неясны требования к программе. Вопрос о требования
можно выразить в виде теста - закомментированного или помеченного аннотацией `@Ignore`.

### T5: Тестируйте граничные условия

Не забывайте о тестировании граничных условий.

### T6: Тщательно тестируйте код связанный с багами

Баги часто собираются группами. Если обнаружится баг в функции, тщательно протестируйте её. Может оказаться, что ошибка
была не одна.

### T7: Закономерности сбоев часто несут полезную информацию

Иногда анализ закономерностей в сбоях тестовых сценариев помогает выявить причины возникших проблем. Всесторонние тест
кейсы, упорядоченные логичным образом, выявляют закономерности.

### T8: Закономерности покрытия кода часто несут полезную информацию

Анализ того, какой код выполняется или не выполняется в ходе тестирования, иногда подсказывает причины возможных сбоев в
ходе тестирования.

### T9: Тесты должны работать быстро

Медленный тесты не выполняется за разумное время. Они первые в очереди на удаление. Сделайте все необходимое для того,
чтобы тесты работали быстро.

# 18 Многопоточность II

## 18.1 Пример приложения "клиент/сервер"
...
Класс возвращает содержимое одной страницы по URL-адресу:
...
Класс - итератор, предоставляющий содержимое страниц на основании итератора URL-адресов:
...
public class PageIterator {
    ...
    public synchronized String getNextPageOrNull() {
        if (urls.hasNext()) getPageFor(urls.next());
        else return null;
    }
...
Блок `synchronized` очень мал - старайтесь синхронизировать как можно меньший объем кода.

_Более подробный разбор и графики в см. в книге._

### 18.5.1 Вычисление производительности в однопоточной модели

При обработке N страниц в однопоточной модели общее время выполнения составляет - 1,5 секунды * N.

### 18.5.2 Вычисление производительности в многопоточной модели

Если страницы могут загрузиться в произвольном порядке и обрабатываться независимо друг от друга, то для повышения
производительности можно воспользоваться многопоточной моделью. При этом две станицы обрабатываются за одну секунду, что
втрое выше однопоточной модели.

## 18.6 Взаимная блокировка

Для возникновения взаимной блокировки необходимы четыре условия:

### 18.6.1 Условия возникновения

#### 18.6.1.1 Взаимное исключение

Взаимное исключение возникает в том случае, когда несколько потоков должны использовать одни и те же ресурсы, при этом
эти ресурсы:

* не могут использовать несколькими потоками одновременно
* существуют в ограниченном количестве

#### 18.6.1.2 Блокировка с ожиданием

Когда один поток захватывает ресурс, он не освобождает его до тех пор, пока не захватит все остальные необходимые
ресурсы и не завершит работу.

#### 18.6.1.3 Отсутствие с вытеснением

Один поток не может отнимать ресурсы у другого потока. Если один поток захватил ресурс, другой может его получить,
только когда первый его освободит.

#### 18.6.1.4 Циклическое ожидание

Поток T1 захватил ресурс R1, поток T2 захватил ресурс R2. Потоку T1 необходим ресурс R2, а потоку T2 также необходим
ресурс R1.

### 18.6.2 Нарушение условий

Если одно из условий нарушится, то взаимная блокировка исчезнет

#### 18.6.2.1 Нарушение взаимного исключения

* Использовать ресурсы, поддерживающих многопоточный доступ, например `AtomicInteger`
...
Большинство ресурсов существенно в ограниченном количестве и не поддерживают многопоточность. Также, второй ресурс
нередко определяется только по результатам обработки первого ресурса.

#### 18.6.2.2 Нарушение блокировки с ожиданием

Проверять каждый ресурс перед захватом, если какой-то ресурс занят, освободить все захваченные ресурсы и начать заново.

Потенциальные проблемы:

* Истощение - один поток стабильно не может захватить все необходимые ему ресурсы
* Обратимая блокировка - несколько потоков "входят в клинч": все они захватывают один ресурс, затем освобождают один
  ресурс и так по-кругу

Обе ситуации приводит к снижению производительности.

#### 18.6.2.3 Нарушение отсутствия вытеснения

Когда поток обнаруживает, что ресурс занят, он обращается к владельцу с запросом на его освобождение. Если владелец
также ожидает другого ресурса, он освобождает все удерживаемые ресурсы и начинает захват заново.

#### 18.6.2.4 Нарушение циклического ожидания

Самая распространенная стратегия. Она требует систему правил, которые должны соблюдать все стороны. Как правило, это
выполннение порядка захвата и освобождения ресурсов.

Потенциальные проблемы:

* Порядок захвата, может не соответствовать порядку освобождения и ресурс, захваченный в начале, моет не использоваться
  до самого конца
* Соблюдение фиксированного порядка захвата ресурсов возможно не всегда. Если идентификатор второго ресурса определяется
  на основании результата захвата первого ресурса

## 18.7 Тестирование многопоточного кода

Простой тест многопоточного кода может выполняться долго и не факт выявит ошибку.

Тестирование методом Монте-Карло:

* Сделать тесты достаточно гибкими, чтобы можно легко вносить изменения в конфигурацию
* Повторять методы снова и снова со случайным набором параметров
* Писать эти тесты на ранней стадии, чтобы как можно раньше начать их выполннение на сервере
* Сохранить комбинацию параметров при сбое
* Выполняйте тесты на каждой целевой платформе. Многократно. Непрерывно
* Запускать тесты на машинах с разной нагрузкой

## 18.8 Средства тестирования многопоточного кода

_Рекомендуется программа от IBM ConTest. Нужно проверить, мало информации в интернете, возможно не актуальна._
