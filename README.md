...
Есть подход, когда константы размещают в интерфейс и делают реализацию от этого класса, что бы при использовании
...
В коде выше название метода слишком общее. И когда в функции `doRename` вызывается функция `renamePage` - это усугубляет
